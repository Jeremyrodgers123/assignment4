...
void classifyShape( std::ofstream& outputStream, std::vector<std::pair<int, int> >& shape){
...
void mainClassifier(std::fstream &inputStream,  std::ofstream& outputStream  ){
    std::string line;
    while(std::getline(inputStream, line)){
        ...
        if(isError1(stringToArr(line))){
            ...
            if(isError2(shape)){
                ...
                outputStream << "error 2" << std::endl;
                exit(0);
            }else if (isError3(shape)){
                //std::cout << "error 3" << std::endl;
                outputStream << "error 3" << std::endl;
                exit(0);
            }else if(isError4(shape)){
                //std::cout << "error 4" << std::endl;
                outputStream << "error 4" << std::endl;
                exit(0);
            }else{
                classifyShape(outputStream, shape);
            }
        }
    }
}
...
int main(int argc, const char * argv[]) {
    ...
    mainClassifier(inputStream, outputStream);
}
